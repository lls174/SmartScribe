# SmartScribe - AI 引导、人类把关的网文共创伙伴

一款以设定质量为先、由灵感/写作/审查三智能体辅助的网文创作工具。
...
> 后端当前运行入口为 `backend/src/app.ts`，开发、构建、测试和数据库脚本均以 TypeScript `src` 为准。`backend/` 根目录下的同名 JavaScript 目录与入口仅为历史兼容文件，已废弃，不应作为新代码或测试的导入来源。
...
升级到创作向导与三智能体版本时，执行幂等迁移。开发环境如果不需要旧数据，也可以删除开发数据库后重新执行初始化。

```bash
cd backend
npm run db:migrate:human-collaboration
```

全新部署或需要按固定顺序补齐全部结构时，可执行：
...
3. **新建小说** → 进入创作向导
4. **灵感定位** → AI 提供候选方案，人工采纳或修改
5. **世界观与人物** → AI 逐字段提案，人工确认后写入小说记忆
6. **故事大纲** → AI 基于已确认设定生成草稿
7. **章节写作** → 写作智能体读取人物卡、大纲和前文概括生成正文
8. **审查与定稿** → 审查智能体给出问题清单，用户决定是否返修并最终保存
