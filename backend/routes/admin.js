...
router.post('/bootstrap',
  ...
  async (req, res) => {
    try {
      ...
      res.status(500).json({ message: '初始化管理员失败' })
    }
  }
)
...
router.get('/usage/summary', async (req, res) => {
  try {
    ...
    console.error('获取系统用量失败:', error)
    res.status(500).json({ message: '获取系统用量失败' })
  }
})
...
router.get('/usage/logs', async (req, res) => {
  try {
    ...
    console.error('获取 AI 请求日志失败:', error)
    res.status(500).json({ message: '获取 AI 请求日志失败' })
  }
})
