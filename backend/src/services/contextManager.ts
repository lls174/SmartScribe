...
interface ContinuePromptParams {
  customPrompt?: string
  wordCount?: string
}

interface PolishPromptParams {
  customPrompt?: string
  content?: string
}
...
class ContextManager {
  ...
  buildNovelContext(
    novelMeta?: NovelContext['novelMeta'] | null,
    ...
    if (novelMeta) {
      parts.push({ priority: 10, content: this.formatNovelMeta(novelMeta), label: '小说元信息' })
    }

    if (novelMemory && novelMemory.trim()) {
      parts.push({ priority: 11, content: novelMemory, label: '小说记忆' })
    }
    ...
    if (chapters && chapters.length > 0) {
      ...
      for (let i = 0; i < chapters.length; i += 1) {
        ...
        if (isCurrent) {
          parts.push({ priority: 9, content: this.formatChapterFull(chapter, i), label: `当前章节(第${i + 1}章)` })
        } else if (isRecent) {
          parts.push({ priority: 7, content: this.formatChapterSummary(chapter, i), label: `相邻章节(第${i + 1}章)` })
        } else {
          parts.push({ priority: 3, content: this.formatChapterOutline(chapter, i), label: `远距章节(第${i + 1}章)` })
        }
      }
    }
  ...
  assembleContext(parts: ContextPart[]): string {
  ...
  formatNovelMeta(meta: NonNullable<NovelContext['novelMeta']>): string {
    let result = '【小说信息】\n'
  ...
  formatChapterFull(chapter: Partial<Chapter>, index: number): string {
  ...
  formatChapterSummary(chapter: Partial<Chapter>, index: number): string {
    let result = `【第${index + 1}章：${chapter.title || '未命名'}】(相邻章节-摘要)\n`
    if (chapter.plot) {
      result += `剧情摘要：${chapter.plot}\n`
    }
    if (chapter.content) {
      result += `内容概要：${this.extractSummary(chapter.content, 300)}`
    }
    return result
  }

  formatChapterOutline(chapter: Partial<Chapter>, index: number): string {
  ...
  extractSummary(text: string, maxChars: number): string {
    if (!text) return ''
    if (text.length <= maxChars) return text
  ...
  buildSystemPrompt(task: TaskType): string {
...
- 控制节奏张弛有度`
  ...
  buildGenerationPrompt(params: GenerationPromptParams): string {
  ...
  buildContinuePrompt(params: ContinuePromptParams): string {
  ...
  buildPolishPrompt(params: PolishPromptParams): string {
