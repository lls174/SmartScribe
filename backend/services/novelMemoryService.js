const { Novel, CharacterCard, NovelSetting, Chapter } = require('../models')

class NovelMemoryService {
  async getNovelMemory(novelId, userId) {
  ...
  formatNovelMemory(memory) {
    ...
    if (memory.characters?.length) {
      const characterLines = memory.characters.map((card, index) => {
        ...
        return `${index + 1}. ${card.name}${details.length ? `\n${details.join('\n')}` : ''}`
      })
...
module.exports = new NovelMemoryService()
